...
- 当前 `openai` Provider 已落地配置校验和非流式成功路径：`AI_BASE_URL` 和 `AI_API_KEY` 缺失或空白时返回统一 `AI_PROVIDER_ERROR`；成功路径通过项目自有 HTTP client 契约发送 `POST {AI_BASE_URL}/chat/completions` 并解析 `choices[0].message.content`。
...
- `OpenAiChatProvider` 通过项目自有 `OpenAiChatHttpClient` 契约发送请求，生产实现使用 JDK `HttpClient`，测试使用可控替身。
- 请求路径按 `{AI_BASE_URL}/chat/completions` 拼接，并覆盖 `AI_BASE_URL` 尾部 `/` 规范化。
- 请求头包含 `Content-Type: application/json` 和 `Authorization: Bearer <AI_API_KEY>`；日志捕获测试覆盖成功路径不输出 API Key、Authorization header 和完整用户输入。
- 请求体包含 `model` 和当前用户消息 `messages=[{role:user, content:<message>}]`。
- `AI_REQUEST_TIMEOUT` 进入 HTTP client 调用契约。
- 成功响应读取 `choices[0].message.content` 并映射到 `ProviderChatResponse.content`。
- Controller 集成测试覆盖 `AI_PROVIDER=openai` 成功时返回 `provider=openai`、`model`、`content`、`traceId`。

下一片继续使用本地 stub HTTP 服务或可控 HTTP 客户端替身，覆盖 OpenAI-compatible Provider 的外部错误映射。
...
- 当前 openai Provider 已覆盖缺配置错误分支和非流式成功路径；外部错误映射继续使用 mock/stub 覆盖。
