...
模型训练、算法实验、数据科学探索仍然以 Python 生态为主。企业 AI 应用落地可以把 Java 作为主力技术栈。
...
AI 项目中继续保留 Java 后端优势：主系统用 Java，模型和数据实验用 Python 辅助。
...
| 密钥、令牌、身份证、银行卡 | 仅在密钥管理或业务系统中保存和使用，不写入 Prompt 或日志 |

模型服务选型同时评估效果、价格、数据处理条款、地域、日志策略和企业合规能力。
...
说明：这个周期是投入较充分时的下限。18 周左右可以做出像样的原型和个人作品集；生产级 AI 平台仍需要更长时间验证稳定性、安全性和治理能力。
...
6. 打印结果。
7. 记录请求耗时和异常。
8. 明确哪些数据可以发给模型，哪些数据必须留在本地或私有环境。
...
做出一个稳定的 AI 聊天后端接口，覆盖接口、异常、日志和基础会话能力。
...
很多 AI 应用需要模型输出 JSON、枚举、列表、分类结果、风险标签，然后由后端继续处理。
...
rerank 由检索后的独立阶段负责，`pgvector` 只做向量检索。rerank 通常需要独立的重排模型，例如：
...
中文知识库一定要重视 embedding 和 rerank 模型。英文效果好的 embedding 模型，需要用中文资料重新验证。
...
学习阶段先用轻量状态机，把 Agent 的状态、边界和确认机制做清楚；业务复杂后再引入重型工作流引擎。
...
先做辅助分析型 Agent。自动改代码、自动发版、自动操作生产数据留到安全边界和人工确认机制成熟之后。
...
1. 继续发挥 Java 后端优势。
2. 先学习 AI 应用集成，再学习模型训练。
3. 先把 Chat、结构化输出、RAG、Tool Calling 做扎实。
4. 中文 RAG 必须重视 embedding 和 rerank 选型。
5. 数据合规和权限边界要从第一天开始考虑。
6. Agent 从半自动和人工确认开始，全自动执行留到安全边界成熟之后。
7. 声称效果变好前，先准备评测集并运行回归。
8. 生产级 AI 应用需要日志、审计、限流和成本统计。
